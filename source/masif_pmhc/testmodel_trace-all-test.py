...
tfrecord_path = "/data/alcohol/MaSIF_pMHC/tfrecords/all-test/"
testing_data = tf.data.TFRecordDataset(
    os.path.join(tfrecord_path, f"{gamenum}_testing_data_6.tfrecord")
)
testing_data = testing_data.map(_parse_function)

model_dir = params["model_all-test"]
output_model = model_dir + f"{gamenum}_model"


## 溯源改动1：结果文件夹
test_set_out_dir = params["predict_trace-all-test"]
