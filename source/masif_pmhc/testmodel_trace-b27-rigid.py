...
tfrecord_path = "/data/alcohol/MaSIF_pMHC/tfrecords/b27-rigid/"
testing_data = tf.data.TFRecordDataset(
    os.path.join(tfrecord_path, f"{gamenum}_testing_data_10.tfrecord")
)
testing_data = testing_data.map(_parse_function)


model_dir = params["model_b27-rigid"]
output_model = model_dir + f"{gamenum}_model"


## 溯源改动1：结果文件夹
test_set_out_dir = params["predict_trace-b27-rigid"]
...
with tf.Session() as sess:
    ...
    for num_test_sample in range(num_test_samples):
        ...
        for ligand in range(n_ligands):
            ...
            pocket_labels = np.zeros(6, dtype=np.float32) ## 注意修改class数量
