...
training_data = tf.data.TFRecordDataset(
    os.path.join(params["tfrecords_b27-rigid"], f"{gamenum}_training_data_20.tfrecord")
)
...
testing_data = tf.data.TFRecordDataset(
    os.path.join(params["tfrecords_b27-rigid"], f"{gamenum}_testing_data_10.tfrecord")
)
...
out_dir = params["model_b27-rigid"]
...
with tf.compat.v1.Session() as sess:
    ...
    for num_epoch in range(num_epochs):
        num_training_samples = 20 ## 注意修改
        #num_validation_samples = 120
        num_testing_samples = 10 ## 注意修改 #9.26
