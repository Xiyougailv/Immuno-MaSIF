...
testing_data = tf.data.TFRecordDataset(
    os.path.join(params["tfrecords_dir"], "testing_data_6.tfrecord")
)
...
with tf.Session() as sess:
    ...
    for num_test_sample in range(num_test_samples):
        ...
        for ligand in range(n_ligands):
            ...
            pocket_points = np.where(labels[:, ligand] != 0.0)[0] ## 所有patch-center-pool中points的索引
            ...
            for i in range(100):
                ...
                ## 解释性改动1
                ###  把本次随机选取的32个patch center的索引保存在.csv中（追加保存）
