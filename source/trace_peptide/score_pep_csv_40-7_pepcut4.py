...
sum_csv_file = os.path.join(csv_dir, "scorepep_sum.csv")
weigh_sum_csv_file = os.path.join(csv_dir, "weigh_scorepep_sum.csv")

extc_ratio = 0.10 ## 抽取行号百分比 ## 24.Dec

### 处理标签
labels_dict = {"A6": 0, "1E6": 1, "DMF5": 2, "JM22": 3,"a24b17": 4, "T4H2": 5, "868": 6} ### 40-7 24.Dec.17，注意最后两个标签顺序，与.csv文件呼应
...
## 24.Dec 发现有两个结构（3D39/3D3V）被处理为8个氨基酸
# 提取残基序号和残基名称（去重）
pep_info = t[['residue_number', 'residue_name']].drop_duplicates()
# 转换为字典形式
pep_dict = dict(zip(pep_info['residue_number'], pep_info['residue_name']))
print(f"** 0. Peptide各号位氨基酸依次为：\n {pep_dict} \n")
