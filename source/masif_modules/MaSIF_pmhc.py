...
class MaSIF_pmhc:
    ...
    def inference(
        ...
        n_samples = tf.shape(rho_coords)[0] ## batch_size == 32
        ...
        for k in range(self.n_rotations):
            ...
            ## 240711：这里的batch_size就是抽取的 32 块patch特征数据
            ## n_feat = 1 -> 只取一种特征
            ## n_gauss = 80
            ...
            )  # batch_size, n_vertices, n_feat, n_gauss,   ## 特征与权重参数相乘：vij(x, x')f(x')            
            gauss_desc = tf.reduce_sum(gauss_desc, 1)  # batch_size, n_feat, n_gauss, ## 240711: 对每个patch内的所有vertice做求和操作，文章中公式右边的求和符号在这里体现
            gauss_desc = tf.reshape(
                gauss_desc, [n_samples, self.n_thetas * self.n_rhos]
            )  # batch_size, 80  ## 240711: 对应文中公式左边 Dj(x)f 
            ## 对于每一个"soft patch"Dij(x)（即由rho和theta定位的每一个bin），对patch内的每个vertice求和
        ...
        conv_feat = tf.reduce_max(all_conv_feat, 0)  ## (batch_size, 80)
        ## 240711：对应于本patch的本feature的某filter，在16个值里挑最大值
        ## 当然，这些最大值分散在16种rotation中
        #### ----------------- ####
        ##        Relu 层
        #### ----------------- ####
    ...
    def __init__(
        ...
        with tf.get_default_graph().as_default() as g:
            ...
            for pr in range(1):
                ...
                initial_coords = self.compute_initial_coordinates() ## (80, 2)
                ...
                for i in range(self.n_feat): 
                    ...
                    my_input_feat = tf.expand_dims(self.input_feat[:, :, i], 2) ## 240711: ~[32, 200, 1, 1]

                    # self.flipped_theta_coords = 0*self.theta_coords;

                    ################ 
                    # 240711: 这里看到：一种特征，对应一套权重参数，shape:(80,80)
                    #         我们同时看到，对于特定的一种特征，32个patch之间共享一套权重参数
                    #         （由于在inference函数中，可以看到所有张量的第一维度均为batch_size，即32）
                    ################
                ...
                #########################
                ## 240711: 这里代表，将1个patch的5种特征通道通过全连接层合并
                ## 维度变化为(32, 5*80) --> (32, 80)
                #########################
                self.global_desc_1 = tf.compat.v1.layers.dense(
                    self.global_desc_1,
                    self.n_thetas * self.n_rhos,
