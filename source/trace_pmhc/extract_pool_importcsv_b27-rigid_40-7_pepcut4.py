...
csv_dir = params["predict_trace_40-7_pepcut4_b27-rigid"] ## 24.Dec ## 包含：我们为了溯源重新test的.npy文件 & 记录每个结构sample/logits的.csv大表
...
csv_out_file = os.path.join(csv_dir, f'C6pred_pool_{pdb}.csv') ## 0308 ## 溯源指定标签：Ln --> Label n 
weigh_csv_out_file = os.path.join(csv_dir, f'C6pred_weigh_pool_{pdb}.csv')

## 处理标签
labels_dict = {"A6": 0, "1E6": 1, "DMF5": 2, "JM22": 3,"a24b17": 4, "T4H2": 5, "868": 6, "newb27": 7} ## 24.Dec ## 与.csv文件的索引格式呼应：从0开始

corr_label = 5 ## 溯源指定标签：C5--a24b17 && C6--T4H2 ## 24.Dec
...
with open(logits_path, 'r') as lf:
    # 读取第5001-10000行
    df = pd.read_csv(lf, header=None, skiprows=5000, nrows=5000)
'''
...
print("** 2. 将logits.csv按照置信度排序，并抽取前10% (24.Dec)：")
print(df_extc)
