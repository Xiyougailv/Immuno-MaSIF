...
def computeCharges(pdb_filename, vertices, names):
    ...
    for ix, name in enumerate(names):
        ...
        ## 参数格式：距离此vertice最近的原子名称；最近原子所在的残基（类别等？）；所有vertice的3D坐标
...
def computeChargeHelper(atom_name, res, v):
    ...
    ## 查依据不同氨基酸种类给定的polarHydrogens表，如果：
    ## 距离此vertice最近的原子为“极性的氢原子”
    if isPolarHydrogen(atom_name, res):
        """
        Donor：主链N-H Or 侧链N-H/O-H，Apr 2025
...
def computeAnglePenalty(angle_deviation):
    ...
    ## hbond_std_dev：hbond_std_dev = np.pi / 3（60度）
