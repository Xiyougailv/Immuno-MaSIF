...
def compute_polar_coordinates(mesh, do_fast=True, radius=12, max_vertices=200):
    ...
    ##################################
    ## 用networkx，以vertice为node, 以欧几里得距离weight edge
    ##################################
    # Graph 
    G=nx.Graph()
    n = len(mesh.vertices)
    ...
    ##################################
    ## P.S. 用dijkstra方法，以连接两vertice(node)的相邻图边之和近似作为测地线距离
    ## nx.all_pairs_dijkstra_path_length：计算全部node之间的最少权重途径长短
    ## dists: 格式：a dictionary of dictionaries（即字典嵌套）
    #         (第一个字典以source node为key，对应的key值为一:
    #           以cutoff范围内各个target node为key，两node间距离为key值的字典)
    ## d2: 
    ## D: 稀疏矩阵：(row, col, data)-->(source node, target node, distance)
    ##################################    
    start = time.perf_counter() ## python旧版：start = time.clock()
    if do_fast:
    ...
    for key_tuple in dists:
        d2[key_tuple[0]] = key_tuple[1]  ## 没有理解？？？
    ...
    #########################################
    ## 只取出距离中心vert最近的max_vertices数目的verts，
    ##   存储其测地极坐标信息
    #########################################
    n = len(d2)
    ...
    for i in range(n): 
        dists_i = d2[i]
        sorted_dists_i = sorted(dists_i.items(), key=lambda kv: kv[1]) ##按照距离数值排序（升序）
...
def compute_thetas(plane, vix, verts, faces, normal, neighbors, idx):
    ## 参数格式：范围内node展成的平面；中心node的索引；vertices; faces; normals; 范围内node的索引；faces的索引
    """
    ...
    """
    ########### plane: 储存了MDS展成的二维平面内各个原node对应映射点的坐标
    ...
    vecs = np.divide(plane,norm_plane) ## 归一化：将plane中的元素逐个除以norm_plane中的对应元素
    ### vecs: 二维的展平面中所有点的归一化（相对中心点）坐标（即：此时所有归一化点以中心node向外以半径1 辐射）
    ...
    term1 = np.sqrt(np.sum(np.square(np.cross(vecs,ref_vec)),axis=1)) ## 叉乘得到的向量的模长：|A||B|sinθ
    term1 = np.arctan2(term1, np.dot(vecs,ref_vec))    ## 点乘得到的向量的模长：|A||B|cosθ；arctan2(x,y) = arctan(y/x)-Pi(???可能有问题)
    ...
    ###################################################
    ## 上述代码总结：
    # Step1: 准备工作：挑选一个参考vertice(方向)；将展平面所有点相对中心点归一化（中心点位于(0,0)）；
    # Step2: 计算角度：计算所有点(矢量)相对参考方向的角度；
    # Step3: 后处理：确保在2D展平平面内计算的角度方向与3D空间的角度方向一致
    # 返回：Patch内所有vertice的角坐标（theta值）
    ###################################################
    return thetas
...
def call_mds(mds_obj, pair_dist):
    ...
    ######### 返回：二维空间中，各个映射点的坐标（参看https://news.sangniao.com/p/2253699536）
...
def compute_theta_all_fast(D, vertices, faces, normals, idx, radius):
    ...
    ###################
    ## MDS参数解释：
    # n_component: 缩放目标维度；n_init: 以不同初始化运行的次数；
    ...
    for i in range(D.shape[0]):
        ############################################################
        ## D: 稀疏矩阵：(row, col, data)-->(source node, target node, distance)
        ## pair_dist_i：存储了所有在node i周围radius/2范围内的所有node相互之间的测地线距离
        ### 这段代码的细节还没有深究清楚？？？
        ############################################################
        # Get the pairs of geodesic distances.
        neigh = D[i].nonzero()                      
        ...
        pair_dist_i = D[neigh_i,:][:,neigh_i]
        pair_dist_i = pair_dist_i.todense()
        pair_dist_i = np.asarray(pair_dist_i) ## 将矩阵转换成数组，SC
        ...
        # Compute the angles on the plane.
        theta = compute_thetas(plane_i, i, vertices, faces, normals, neigh_i, idx)
        ## 参数格式：范围内node展成的平面（各个原node对应映射点的坐标）；中心node的索引；
        ##         vertices; faces; normals; 范围内node的索引；faces的索引

        #######################################
        ## patch内距中心vert大于radius/2范围中的verts的角坐标为：
        #      与它们各自最接近的（小于radius/2范围内的某个）vert的角坐标一致
        #######################################
