...
training_data = tf.data.TFRecordDataset(
    os.path.join(params["tfrecords_40-7_pepcut4_b27-rigid"], f"{gamenum}_training_data_40.tfrecord") ## For 40-7, 24.Dec
)
...
testing_data = tf.data.TFRecordDataset(
    os.path.join(params["tfrecords_40-7_pepcut4_b27-rigid"], f"{gamenum}_testing_data_6.tfrecord") ## For 32-6_Infer-6
)
...
out_dir = params["model_40-7_pepcut4_b27-rigid"] ## 3.3
...
with tf.compat.v1.Session() as sess:
    ...
    total_iterations = 0
    num_epochs = 500 ##### Longer training epochs, 24.Dec
    for num_epoch in range(num_epochs):
        num_training_samples = 40 ## 注意修改 32-6
        #num_validation_samples = 120
        num_testing_samples = 6 ## 注意修改 12.21
        ...
        for num_sample in range(num_training_samples):
            ...
            pocket_labels = np.zeros(8, dtype=np.float32) ################### 注意修改class数量 12.22 For 29-6 & a FAKE class.
