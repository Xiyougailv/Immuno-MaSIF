...
####    /data/alcohol/MaSIF_pMHC/tfrecords/all-test/文件夹下
tfrecord_path = params["tfrecords_40-7_pepcut4_all-test"] ## 0305，改动（或params["tfrecords_31-6_all-test"]）
testing_data = tf.data.TFRecordDataset(
    os.path.join(tfrecord_path, f"{gamenum}_testing_data_13.tfrecord") ## 0305，改动
)
testing_data = testing_data.map(_parse_function)

model_dir = params["model_40-7_pepcut4_all-test"] ## 0305
output_model = model_dir + f"{gamenum}_model"


## 溯源改动1：结果文件夹
test_set_out_dir = params["predict_trace_40-7_pepcut4_all-test"] ## 0305
...
with tf.Session() as sess:
    ...
    for num_test_sample in range(num_test_samples):
        ...
        for ligand in range(n_ligands):
            ...
            pocket_labels = np.zeros(7, dtype=np.float32) ## 注意修改class数量 For 31-6, 0305
        ...
        np.save(test_set_out_dir + f"{gamenum}_{pdb}_labels.npy", pdb_labels)
