...
if not os.path.exists(masif_opts["pmhc_fingerprint"]["tcr_coords_dir_clustmd"]):
    os.mkdir(masif_opts["pmhc_fingerprint"]["tcr_coords_dir_clustmd"])
...
parser = PDBParser()
pdb_file = masif_opts['raw_pdb_dir']+"Clust_MD/"+pdb_info+"/"+pdb_id+"_"+nf+"_"+f+".pdb"  ## 原始PDB位置
...
np.save(
    os.path.join(
        masif_opts["pmhc_fingerprint"]["tcr_coords_dir_clustmd"], "{}_tcr_type_{}_{}.npy".format(pdb_id, nf, f)
    ),
    structure_tcr_type,
)
np.save(
    os.path.join(
        masif_opts["pmhc_fingerprint"]["tcr_coords_dir_clustmd"], "{}_tcr_coords_{}_{}.npy".format(pdb_id, nf, f)
    ),
    structure_tcr_coords,
)
