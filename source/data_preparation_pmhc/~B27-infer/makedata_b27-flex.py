...
## 需要增加一类”标签“
tcr_labels = ["A6", "JM22", "868", "1E6", "a24b17", "newb27"]
# List all structures that have been preprocessed
## 这里为了简便起见，没有更换搜寻位置（但本case下需要用到/MD/子文件夹下的数据）
...
selected_pdbs = []
## 修改范围名单 # 9.26 修改名单文件名
f = open("./data_preparation_pmhc/b27_list_30.txt", "r")
...
labels_dict = {"A6": 1, "JM22": 2, "868": 3, "1E6": 4, "a24b17": 5, "newb27": 6}
...
datalist_dir = params["datalist_b27-flex"]
...
## 添加/MD/子文件夹路径  ## 9.26 修改为添加/Clust_MD/子文件夹路径：改用聚类后（注意：为了简化修改，并没有修改本脚本中的变量名：md/clustmd
precom_dir_md = params["masif_precomputation_dir_clustmd"]
tcr_coord_dir_md = params["tcr_coords_dir_clustmd"]

tfrecords_dir = params["tfrecords_b27-flex"]
...
with tf.compat.v1.python_io.TFRecordWriter(
    os.path.join(tfrecords_dir, f"{gamenum}_training_data_20.tfrecord")
) as writer:
    for i, pdb in enumerate(train_pdbs):
        ...
        try:
            ...
            input_feat = np.load(
                os.path.join(precom_dir_md, f"{sum_face}_{face_num}", pdb, "p1_input_feat.npy") ## 删去了"_" ???
            )
            rho_wrt_center = np.load(
                os.path.join(precom_dir_md, f"{sum_face}_{face_num}", pdb, "p1_rho_wrt_center.npy")
            )
            theta_wrt_center = np.load(
                os.path.join(precom_dir_md, f"{sum_face}_{face_num}", pdb, "p1_theta_wrt_center.npy")
            )
            mask = np.expand_dims(np.load(os.path.join(precom_dir_md, f"{sum_face}_{face_num}", pdb, "p1_mask.npy")),-1)
            X = np.load(os.path.join(precom_dir_md, f"{sum_face}_{face_num}", pdb, "p1_X.npy"))
            Y = np.load(os.path.join(precom_dir_md, f"{sum_face}_{face_num}", pdb, "p1_Y.npy"))
            Z = np.load(os.path.join(precom_dir_md, f"{sum_face}_{face_num}", pdb, "p1_Z.npy"))
            all_tcr_coords = np.load(
                os.path.join(
                    tcr_coord_dir_md, "{}_tcr_coords_{}_{}.npy".format(pdb.split("_")[0], sum_face, face_num)
                )
            )
            all_tcr_types = np.load(
                os.path.join(
                    tcr_coord_dir_md, "{}_tcr_type_{}_{}.npy".format(pdb.split("_")[0], sum_face, face_num)
                )
...
success = 0
## 9.26 修改文件名：test十种B27结构
with tf.compat.v1.python_io.TFRecordWriter(
    os.path.join(tfrecords_dir, f"{gamenum}_testing_data_10.tfrecord")
) as writer:
    for i, pdb in enumerate(test_pdbs):
        print("Working on Testing pdb: ", pdb)
        ## ！！对一个PDB文件操作
        ## 由于B27结构还没有跑MD，所以为rigid-face
        try:
            # Load precomputed data
            input_feat = np.load(
