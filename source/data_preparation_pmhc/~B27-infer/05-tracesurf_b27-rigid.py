...
###  溯源B27推断测试（新增了一个"fake"标签）；SC, 2023/11/06
####################################
...
## 溯源：csv来自：结果文件夹 
test_set_out_dir = params["predict_trace_40-7_pepcut4_b27-rigid"] ## 24.Dec ## For B27-rigid
## 溯源：保存pool.ply文件：确保创建ply文件夹
ply_set_out_dir = params["ply_trace_40-7_pepcut4_b27-rigid"] ## 24.Dec ## For B27-rigid
...
for ppi_pair_id in ppi_pair_list:
    ...
    my_precomp_dir = params['masif_precomputation_dir']+ppi_pair_id+'/' ## 溯源时，没有用到此文件夹
    ...
    csv_file['p1'] = test_set_out_dir + "/C6pred_pool_{}.csv".format(fields[0])  ## 24.Dec ## 11.06 Label 2：L2pred_pool_5C07.csv，下同
    ### 添加权重，23/10/27
    weigh_csv_file = {}
    # weigh_csv_file['p1'] = test_set_out_dir + "/weigh_pool_{}.csv".format(fields[0])  ## 本标签：weigh_pool_5C07.csv
    weigh_csv_file['p1'] = test_set_out_dir + "/C6pred_weigh_pool_{}.csv".format(fields[0])  ## 0308 ## 10.30 非本标签：maxpred_weigh_pool_5C07.csv

    out_ply_file = {}
    #out_ply_file['p1'] = ply_set_out_dir + "/pool_{}.ply".format(fields[0]) ## pool_5C07.ply
    out_ply_file['p1'] = ply_set_out_dir + "/C6pred_pool_{}.ply".format(fields[0]) ## 0308 ## 10.30 非本标签：maxpred_pool_5C07.ply
    ### 添加权重，23/10/27
    weigh_out_ply_file = {}
    #weigh_out_ply_file['p1'] = ply_set_out_dir + "/weigh_pool_{}.ply".format(fields[0]) ## pool_5C07.ply
    weigh_out_ply_file['p1'] = ply_set_out_dir + "/C6pred_weigh_pool_{}.ply".format(fields[0]) ## 0308 ## 10.30 非本标签：maxpred_weigh_pool_5C07.ply
