...
pdb_id = in_fields[0]
chain_ids1 = in_fields[1] ## Chain AC for pMHC
chain_ids2 = in_fields[2] ## Chain DE for TCR
chain_cplx = chain_ids1 + chain_ids2 ## ACDE for cplx (Label the interface)
...
## Extract complex for interface labels.
out_filename2 = tmp_dir+"/"+pdb_id+"_"+chain_cplx
extractPDB(pdb_filename, out_filename2+".pdb", chain_cplx)
...
regular_mesh = fix_mesh(mesh, masif_opts['mesh_res']) ## pMHC
...
if 'dmasif_compute_iface' in masif_opts and masif_opts['dmasif_compute_iface']:
    # Compute the surface of the entire complex and from that compute the interface.
    ##v3, f3, _, _, _ = computeMSMS(pdb_filename, protonate=True) ## ~550
    v3, f3, _, _, _ = computeMSMS(out_filename2+".pdb", protonate=True) ## ~300
    # Regularize the mesh
    ...
    kdt = KDTree(v3)
    d, r = kdt.query(regular_mesh.vertices) ## 240921：查找单体表面mesh每个点与完整复合物表面mesh的点之间的距离
    ...
    iface[iface_v] = 1.0
    print(f"The number of vertices labeled for the TCR-pMHC interface is {len(iface_v)} :)")
    ...
    save_ply(out_filename1+".ply", regular_mesh.vertices,\
                        regular_mesh.faces, normals=vertex_normal, charges=vertex_charges,\
                        normalize_charges=True, hbond=vertex_hbond, hphob=vertex_hphobicity)
if not os.path.exists(masif_opts['dmasif_ply_chain_dir']):
    os.makedirs(masif_opts['dmasif_ply_chain_dir'])
if not os.path.exists(masif_opts['dmasif_pdb_chain_dir']):
    os.makedirs(masif_opts['dmasif_pdb_chain_dir'])
shutil.copy(out_filename1+'.ply', masif_opts['dmasif_ply_chain_dir']) 
shutil.copy(out_filename1+'.pdb', masif_opts['dmasif_pdb_chain_dir']) 
