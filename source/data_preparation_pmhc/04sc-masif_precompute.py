...
for ppi_pair_id in ppi_pair_list:
    ...
    for pid in pids: 
        ...
        ## Size: ~[5000, 200] --> [surface vertices, patch max size]
        np.save(my_precomp_dir+pid+'_theta_wrt_center', theta[pid])
        np.save(my_precomp_dir+pid+'_input_feat', input_feat[pid]) 
        ## Size: ~[5000, 200, 5] --> [surface vertices, patch max size, feature channels]
        ## 240711 注意：patch中的vertice数量可能小于设定值200，因为第二个限制条件是cutoff为 12 Angstrom
